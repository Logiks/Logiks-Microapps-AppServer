...
module.exports = {
    ...
    processJSONComponent: async function(jsonObj, objId, moduleId, ctx) {
        ...
        if(jsonObj.policy && jsonObj.policy.length>0) {
            var isAllowed = await checkAccessPolicy(ctx, jsonObj.policy, {type: moduleId, objId, moduleId, ...jsonObj});//false
            if(!isAllowed) {
            ...
            switch(moduleId) {
                ...
                case "infoview":
                    if(tempObj.source && tempObj.source.type=="sql") {
                        ...
                        const dbOpsID = await DBOPS.storeDBOpsQuery(tempObj.source, tempObj.fields, operationId, tempObj.forcefill?tempObj.forcefill:{}, ctx.meta.user, {objId, moduleId, "refid": tempObj.source.refid}, tempObj.hooks?tempObj.hooks:{}, ctx);
                        tempObj.source = {
                            "type": "sql",
                            "dbopsid": dbOpsID
                        };
                    ...
                    if(tempObj.actions) {
                        ...
                        for (var i = actionList1.length - 1; i >= 0; i--) {
                            ...
                            if(v.policy && v.policy.length>0) {
                                var isAllowed = await checkAccessPolicy(ctx, v.policy, {type: moduleId, objId, moduleId, ...jsonObj});
                                if(!isAllowed) {
                    ...
                    if(tempObj.infoview && tempObj.infoview.groups) {
                        ...
                        for (var i = groupList.length - 1; i >= 0; i--) {
                            ...
                            if(v.config && v.config.form && v.config.form.source && v.config.form.source.type=="sql") {
                                ...
                                const dbOpsID = await DBOPS.storeDBOpsQuery(v.config.form.source, v.config.form.fields, "fetch", v.config.form.forcefill?v.config.form.forcefill:{}, ctx.meta.user, {objId: newRefID, moduleId, "refid": tempObj.source.refid}, v.config.form.hooks?v.config.form.hooks:{}, ctx);
                            ...
                            if(v.config && v.config['popup.form'] && v.config['popup.form'].source && v.config['popup.form'].source.type=="sql") {
                                ...
                                const dbOpsID = await DBOPS.storeDBOpsQuery(v.config['popup.form'].source, v.config['popup.form'].fields, "fetch", v.config['popup.form'].forcefill?v.config['popup.form'].forcefill:{}, ctx.meta.user, {objId: newRefID1, moduleId, "refid": tempObj.source.refid}, v.config['popup.form'].hooks?v.config['popup.form'].hooks:{}, ctx);
    ...
    processFormFields: async function(formFields, ctx, objId, moduleId) {
        ...
        for (var i = fieldList.length - 1; i >= 0; i--) {
            ...
            try {
                if(v.policy && v.policy.length>0) {
                    var isAllowed = await checkAccessPolicy(ctx, v.policy, {type: "formfield", objId, moduleId, ...formFields, "field": k});
                    if(!isAllowed) {
                        delete formFields[k];
                        continue;
                    }
